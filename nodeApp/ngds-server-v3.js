...
var pg = require('pg'),
    connectionString = 'postgres://user:***@localhost:5432/geothermal',
...
/* Initialization - retrieves pending jobs and procs from the database */
    ...
    function execStack() {
    	...
    	setTimeout(function() {
    		console.log('exec Stack count -- ' + jCnt + ' ' + JSON.stringify(curActProcs));
	...
	function getActivities( caid ) {
		...
		cSql = 'select c.ca_id,c.ca_type, c.action_date, c.end_date, c.set_id, a.agentid, c.status '
	...
	async function getProcessQueue(colActId, directive) {
		...
		return new Promise(function(resolve, reject){
			...
			pgClient.query(qStr, (err, res) => {
				if ( typeof(res) !== "undefined" ) {
					var rowrec = res.rows;
					//console.log('Process jobque '+JSON.stringify(rowrec));
					for ( var k in rowrec) {
						if ( rowrec[k].jobtype == 'node-API' || rowrec[k].jobtype == 'node-api' || rowrec[k].jobtype == 'node-client') {
...
var cf = new autoFunction('cswFetchRecords');
cf.exec = async function(o) {
...
var lc = new autoFunction('linkValidate');
lc.exec = async function(o) {

	function queryLinks(o) {
		var r = o.url_params;
		var rid = r.RuleId;
		var rtype = r.rule_type;

		if ( rtype == 'link' || rtype == 'catalog') {
			var s = 'select * from inspect_rule_links3('+rid+')';
		} else if ( rtype == 'crawl' ) {
			var s = 'select * from resource_inspection where ' 
			         + ' proc_name = \'RunCrawl rule_id-'+rid+'\'';
		} else if ( rtype == 'base') {
			var s = 'select * from resource_links';	         
		}

		return new Promise(function(resolve, reject){
			pgClient.query(s, (err, res) => {
				if ( typeof(res) !== "undefined" ) {
					var rowrec = res.rows;
					for ( var k in rowrec) {
						var ux = rowrec[k];
					...
					console.log('inspect query complete '+rc);
				} else {
		...
		if ( u.lurl.indexOf('//ftp') > 0 ) {
			...
			rex(hurl, {method: 'HEAD'}, function (err, rsp, body){
				if ( typeof(rsp) !== "undefined" && typeof(rsp.statusCode) !== "undefined") {
					...
					console.log('lcheck '+hurl+' '+rsp.statusCode)
					u.rsp = rsp;
					...
					console.log('Inspect link error ' + hurl) ;
					var zx = err;
                    u.err = err;
	...
	function updateCrawlCheck(o) {
		console.log('Inspection save crawl check '+ o.ident_url );
...
var wc = new autoFunction('runCrawler');
wc.exec = async function(o) {
    // o is proces_jobque record
	var r = o.url_params;
	var rid = r.RuleId;

	var s = 'select * from crawl_rule_links('+rid+')' 
			+ ' where identifier not in '
			+ ' (select guid from resource_inspection where proc_name = \'RunCrawl rule_id-'+rid+'\')';
	...
	function crawlPage(u) {
		console.log(' crawl params '+ u.crawl_element+' '+u.rel_path+' '+typeof(u.crawl_element));
		...
		console.log('crawl params ' + u.lurl + ' ' + datatype + ':'+typeclass + ' ' + rv);
		...
		.then(function(links,utils) {
			...
			//console.log('THEN ' + u.lurl + ' ' + u.identifier + linkfilter + dataclass + ' ' + links);
		});
	}
