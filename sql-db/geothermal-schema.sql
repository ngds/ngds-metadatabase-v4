...
--
-- Name: bec_test; Type: TYPE; Schema: public; Owner: ngdsdb
--

CREATE TYPE public.bec_test AS (
	guid text,
	version_id bigint
);


ALTER TYPE public.bec_test OWNER TO ngdsdb;

--
-- Name: becx; Type: TYPE; Schema: public; Owner: ngdsdb
--

CREATE TYPE public.becx AS (
	guid text,
	version_id bigint
);


ALTER TYPE public.becx OWNER TO ngdsdb;

--
-- Name: bqrow; Type: TYPE; Schema: public; Owner: ngdsdb
--

CREATE TYPE public.bqrow AS (
	guid text,
	citation_title text,
	version_id bigint,
	node_id bigint,
...
ALTER TYPE public.bqrow OWNER TO ngdsdb;
...
--
-- Name: schemapvrow; Type: TYPE; Schema: public; Owner: ngdsdb
--

CREATE TYPE public.schemapvrow AS (
	map_id bigint,
	schem_id integer,
	moid bigint,
	fed_elem text,
...
ALTER TYPE public.schemapvrow OWNER TO ngdsdb;
...
--
-- Name: batch_edit_collect(bigint, bigint, json); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.batch_edit_collect(bigint, bigint, json) RETURNS text
    LANGUAGE plpgsql
    AS $_$
DECLARE
...
BEGIN
	...
	FOR ce IN
		--select * from json_array_elements(col)
		select * from json_populate_recordset(null::record,col) 
		as (guid text, citation_title text, version_id bigint,
		   node_id bigint, node_name text , node_value text, mpath text, checked boolean) 
	LOOP    
		update mdvnode set node_value = ce.node_value where node_id = ce.node_id;
		IF NOT FOUND THEN
			insert into cap_records (cpr_id, ca_id, pjq_id, mdv_id, status, guid, modified) 
			values (nextval('cap_req_id_seq'), ca,pjq, ce.version_id,
			'batup-update-error-'||ce.node_name, ce.guid, current_timestamp);
			
		ELSE
		
			insert into cap_records (cpr_id, ca_id, pjq_id, mdv_id, status, guid, modified) 
			values (nextval('cap_req_id_seq'), ca,pjq, ce.version_id,
			'batch-update-'||ce.node_name, ce.guid, current_timestamp);
			-- archive old version - use parent_id to point to current version
			insert into md_version (mdv_id, md_id, version_id, parent_id, 
									status,content_url, source_schema_id, 
									create_date, end_date)
				select nextval('md_version_mdv_id_seq'), md_id, version_id, ce.version_id, 
					'archive', content_url,	source_schema_id, create_date, current_timestamp 
				from md_version where mdv_id = ce.version_id;
			-- update new version 
			update md_version set version_id = version_id+1, create_date = current_timestamp 
			where mdv_id = ce.version_id;
			rc := rc+1;
		END IF;
	...
	IF rc > 0 THEN
		update cap_jobque set status = 'complete' where pjq_id = pjq;
	ELSE
		update cap_jobque set status = '0 records' where pjq_id = pjq;
	END IF;	
	RETURN 'DONE';
END;
$_$;
...
ALTER FUNCTION public.batch_edit_collect(bigint, bigint, json) OWNER TO ngdsdb;

--
-- Name: batch_edit_status(bigint, bigint, bigint, text); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.batch_edit_status(bigint, bigint, bigint, text) RETURNS text
    LANGUAGE plpgsql
    AS $_$
DECLARE
...
BEGIN
		update cap_records set status = status || '>pycsw-'|| statxt where pjq_id = pyjq;
		update cap_jobque set status = statxt where pjq_id = pyjq;
		update collection_activity set status = statxt where ca_id = caid;
		update collections set status = statxt where set_id = setid;
...
ALTER FUNCTION public.batch_edit_status(bigint, bigint, bigint, text) OWNER TO ngdsdb;

--
-- Name: batchguidqry(text, text, text); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.batchguidqry(text, text, text) RETURNS SETOF public.bqrow
    LANGUAGE plpgsql
    AS $_$
DECLARE
	fld ALIAS FOR $1;
...
BEGIN
	...
	FOR rd IN
		...
		( select guid, citation_title, mdv_id, source_schema_id as sid
		 from md_record r, md_version v, zib z
		 where r.md_id = v.md_id and v.end_date is null and r.guid = z.id ),
		mdv as (
			select guid, citation_title, mdv_id as version_id, sid,
			node_id, node_name, trim(both '"' from node_value) node_value, 
			substring(mpath::text, 2, length(mpath::text) -2) as mpath
			from mdview2 m , cv r
			where m.version_id = r.mdv_id and node_value ilike qry
		), slu as (
			select fed_elem, map_path, schema_id from schema_map where fed_elem = fld
		), slo as (
		   select o.fed_elem, o.relmapath, m.schema_id 
			from schema_object_map o, schema_map m
			where o.map_id = m.map_id and o.fed_elem = fld
		), mvmap as (
			select guid, citation_title, version_id, node_id, node_name, node_value, mpath 
			from mdv, slu
			where mpath = map_path
				and schema_id = sid
		), mvom as (
			select guid, citation_title, version_id, node_id, node_name, node_value, mpath 
			from mdv, slo 
			where mpath like relmapath
			and schema_id = sid
		)
		select * from mvmap
		union
...
ALTER FUNCTION public.batchguidqry(text, text, text) OWNER TO ngdsdb;

--
-- Name: batchqry(text, text); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.batchqry(text, text) RETURNS SETOF public.bqrow
    LANGUAGE plpgsql
    AS $_$
DECLARE
	fld ALIAS FOR $1;
...
BEGIN
	...
	FOR rd IN
		...
		mdv as (
			...
			substring(mpath::text, 2, length(mpath::text) -2) as mpath
			from mdview2 m , cv r
			where m.version_id = r.mdv_id and node_value like qry
		), slu as (
			select fed_elem, map_path, schema_id from schema_map where fed_elem = fld
		), slo as (
		   select o.fed_elem, o.relmapath, m.schema_id 
...
ALTER FUNCTION public.batchqry(text, text) OWNER TO ngdsdb;

--
-- Name: bec_t(bigint, bigint, json); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.bec_t(bigint, bigint, json) RETURNS text
    LANGUAGE plpgsql
    AS $_$
DECLARE
...
BEGIN
	rc := 0;
	tmx := 'START';
	--for j in ( select json_to_recordset(col)) 
	
	FOR ce IN

		select * from json_populate_recordset(null::record,col) as (guid text, version_id bigint) 
		    
	LOOP    
	   --return next ce;
	   
	   rc := rc+1;
	   tmx := tmx||ce.guid||ce.version_id;
		--raise notice 'help';
		--raise notice '[%]',  ce.version_id;	 
	END LOOP;
	RETURN tmx;
...
ALTER FUNCTION public.bec_t(bigint, bigint, json) OWNER TO ngdsdb;

--
-- Name: bec_test(bigint, bigint, json[]); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.bec_test(bigint, bigint, json[]) RETURNS text
    LANGUAGE plpgsql
    AS $_$
DECLARE
...
ALTER FUNCTION public.bec_test(bigint, bigint, json[]) OWNER TO ngdsdb;
...
--
-- Name: colactivity(bigint); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.colactivity(set_id bigint) RETURNS TABLE(set_id bigint, schema_id bigint, ca_id bigint, action_date timestamp without time zone, rcount bigint)
    LANGUAGE sql
    AS $$

	with ca as (
		select c.set_id, a.ca_id, 
			c.schema_id, a.action_date
		from collections c, collection_activity a
		where c.set_id = a.set_id and 
			c.set_id = colactivity.set_id
	), rc as (
		select ca_id, count(guid) from cap_records 
		where ca_id in 
			( select ca_id from ca ) 
		group by ca_id
	)
	select ca.set_id, ca.schema_id, 
		ca.ca_id, ca.action_date, rc.count
	from ca, rc where rc.ca_id = ca.ca_id 
	order by action_date desc
$$;


ALTER FUNCTION public.colactivity(set_id bigint) OWNER TO ngdsdb;
...
--
-- Name: geturlcheck(text, text); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.geturlcheck(guid text, url text) RETURNS TABLE(guid text, url text, v_date timestamp without time zone, url_status text, contentlen bigint)
    LANGUAGE sql
    AS $$
	with grf as 
	(select guid as g, 
	 	orig_url as o, 
	 	max(v_date) as v
	 from resource_inspection where guid = getUrlCheck.guid
	 group by guid, orig_url )
	 select guid, orig_url, 
		v_date, 
		url_status, 
		http_cont_len as contentlen
	 from resource_inspection i, grf 
	 where i.guid = grf.g and 
		i.orig_url = grf.o and 
		i.v_date = grf.v
$$;


ALTER FUNCTION public.geturlcheck(guid text, url text) OWNER TO ngdsdb;
...
--
-- Name: md_vms(text, bigint); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.md_vms(guid text, vid bigint) RETURNS TABLE(node_id bigint, node_name text, node_value text, map_path text)
    LANGUAGE sql
    AS $$
with cv as (
		select r.md_id, 
		...
		v.version_id
		from md_record r, md_version v 
		where v.md_id = r.md_id and 
			r.guid = md_vms.guid --'B9FBA06359624F24A0A131E0C4DE489D'
), vx as (
   select 
	CASE WHEN md_vms.vid = 0 
		then max(version_id)
		else md_vms.vid 
	end as vidx 
	from cv group by md_id
), selv as (
	select max(mdv_id) as mvid from cv where 
	version_id in ( select vidx from vx )
), smap as ( 
	select * from schema_map 
	where schema_id = 
		(select sid from cv where mdv_id = ( select mvid from selv) ) 
), som as ( 
	select o.fed_elem, o.relmapath 
	from schema_map m, schema_object_map o 
	where m.map_id = o.map_id and 
		m.schema_id = 
			(select sid from cv 
			 where mdv_id = 
			 ( select mvid from selv) ) 
), mdx as (
	select * from mdv (
		( select mvid from selv ) ) 
), mdvsm as (
	select node_id, 
		fed_elem as node_name, 
		node_value, 
		q.map_path 
	from mdx q, smap s
...
union 
select * from mdvsom
--select node_id, node_name, node_value, map_path
--from mdx
$$;


ALTER FUNCTION public.md_vms(guid text, vid bigint) OWNER TO ngdsdb;
...
--
-- Name: mdv_validate(bigint, bigint, bigint); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.mdv_validate(fsx bigint, vsx bigint, mdvid bigint) RETURNS TABLE(fd text, rsid integer, ruleset_name text, node_id bigint, node_value text, stype text, mpath text)
    LANGUAGE sql
    AS $$
with fv as (
		select fed_elem as fe, ruleset_id  as rsid 
		from schema_map where schema_id = mdv_validate.fsx
	), felem as (
	   select  map_id, fed_elem, map_path, stype, rsid
	   from schema_map, fv 
	   where schema_id = mdv_validate.vsx and
	   fed_elem = fe
	), fm as ( 
		select fd, valrule, ruleset_name from 
		( select distinct(fed_elem) as fd, 
			min(rsid) as valrule 
		  from felem group by fed_elem) z, 
		process_ruleset p 
		where z.valrule = p.rs_id
	), mdv as (
		select 
			node_id, 
			node_name, 
			node_value, 
			map_path as mpath
		from mdv(mdv_validate.mdvid)
	),  vals as (
		select * from mdv, felem 
		where felem.map_path = mdv.mpath
	), fvo as (
		select o.map_id, o.fed_elem as fe, o.ruleset_id as rsid 
		from schema_map m, schema_object_map o
		where m.schema_id = mdv_validate.fsx and 
			m.map_id = o.map_id
	), selemo as (
		select o.moid, o.map_id, 
			o.fed_elem, 
			o.relmapath as map_path, 
			o.omtype as stype
		from schema_map m, schema_object_map o
		where m.schema_id = mdv_validate.vsx and 
			m.map_id = o.map_id 
	), felemo as (
		select * from fvo, selemo 
		where fvo.fe = selemo.fed_elem
	), fmo as (
		select fd, valrule, ruleset_name from 
		( select distinct(fed_elem) as fd, min(rsid) as valrule 
		  from felemo group by fed_elem) z, 
		process_ruleset p 
		where z.valrule = p.rs_id
	), mdvo as (
		select node_id, 
		...
		select * from mdvo, felemo 
		where mdvo.mpath like felemo.map_path
	)
	select fd, valrule, ruleset_name, node_id, node_value, stype, mpath  
		from fm left outer join vals on vals.fed_elem = fm.fd
		--order by valrule, fd
	union
	select fd, valrule, ruleset_name, node_id, node_value, stype, mpath  
		from fmo left outer join valo on valo.fed_elem = fmo.fd
	order by valrule, fd
$$;


ALTER FUNCTION public.mdv_validate(fsx bigint, vsx bigint, mdvid bigint) OWNER TO ngdsdb;
...
--
-- Name: schemapv(bigint); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.schemapv(bigint) RETURNS SETOF public.schemapvrow
    LANGUAGE plpgsql
    AS $_$
DECLARE
	sid ALIAS FOR $1;
	rd schemapvrow%ROWTYPE;
	
BEGIN

	FOR rd IN
		SELECT schema_map.map_id,
            schema_map.schema_id,
            0 AS moid,
            schema_map.fed_elem,
            schema_map.map_path,
            schema_map.stype,
            schema_map.ruleset_id
           FROM schema_map 
		   where schema_id = sid and stype = 'end'
		   order by fed_elem
	...
	FOR rd IN
		with sm as (
			...
			SELECT sm.map_id,
				...
				o.omtype as stype,
				o.ruleset_id
			  FROM schema_object_map o, sm
			  WHERE 
				o.map_id = sm.map_id 
			  ORDER BY map_id, moid
		) 
		select * from sm
		union 
		select * from som
...
ALTER FUNCTION public.schemapv(bigint) OWNER TO ngdsdb;

--
-- Name: scmap_insert(bigint, text, text, text, integer, text); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.scmap_insert(bigint, text, text, text, integer, text) RETURNS text
    LANGUAGE plpgsql
    AS $_$
DECLARE
	sid ALIAS FOR $1;
...
BEGIN
    restxt := 'start';
	if qt = 'schema_map' then
		insert into schema_map 
		(map_id, schema_id, fed_elem, map_path, stype, ruleset_id) 
		values 
		( nextval('schema_map_map_id_seq'),
		  sid,fedelem,mp,etype,vr ) returning map_id into mapid;
		restxt := 'success,schema_map,' || mapid::text;
	else
		select map_id, fed_elem into mapid,pfe 
			from schema_map 
			where schema_id = sid and fed_elem in 
			( select fed_elem 
			  from schema_map 
			  where schema_id = 10 and map_id in 
			  ( select map_id from schema_object_map 
				where fed_elem = fedelem and 
					  map_id in 
					  ( select map_id from 
						schema_map 
						where schema_id = 10)
		...
		if mapid > 0 then 
			insert into schema_object_map 
				(moid, map_id, fed_elem, relmapath, omtype, ruleset_id)	
			values (
			  nextval('schema_map_object_id_seq'),
			  mapid,
			  fedelem,
			  mp,
			  etype,
			  vr
			) returning moid into moi;
			restxt := 'success,schema_object_map,' || moi::text;	
		 else 
			insert into schema_map 
			(map_id, schema_id, fed_elem, map_path, stype, ruleset_id) 
			values 
			( nextval('schema_map_map_id_seq'),
			  sid, pfe,'','object', 13 ) returning map_id into mapid;
			restxt := 'Add parent,schema_map,' || mapid::text;
			...
			restxt := restext ||',success,schema_object_map,'|| moi::text;	
		 end if;
	end if;
	return restxt;
...
ALTER FUNCTION public.scmap_insert(bigint, text, text, text, integer, text) OWNER TO ngdsdb;

--
-- Name: scmap_update(bigint, bigint, bigint, integer, text); Type: FUNCTION; Schema: public; Owner: ngdsdb
--

CREATE FUNCTION public.scmap_update(bigint, bigint, bigint, integer, text) RETURNS text
    LANGUAGE plpgsql
    AS $_$
DECLARE
	sid ALIAS FOR $1;
...
BEGIN
	...
	if moidx = 0 then
		...
		set map_path = mp,
			ruleset_id = vr
		where map_id = mapid returning map_id into mpd;
		restxt := 'update,schema_map,'||mpd::text;
	
	elseif moidx > 0 then
		update schema_object_map 
		set relmapath = mp,
			ruleset_id = vr
		where moid = moidx
		returning moid into mpd;
		restxt := 'update,schema_object_map,'||mpd::text;
	end if;
...
ALTER FUNCTION public.scmap_update(bigint, bigint, bigint, integer, text) OWNER TO ngdsdb;
...
--
-- Name: cv; Type: VIEW; Schema: public; Owner: ngdsdb
--

CREATE VIEW public.cv AS
 SELECT r.md_id,
    r.guid,
    r.dataset_id,
    r.citation_title,
...
ALTER TABLE public.cv OWNER TO ngdsdb;
...
--
-- Name: gislinkupdate; Type: MATERIALIZED VIEW; Schema: public; Owner: ngdsdb
--

CREATE MATERIALIZED VIEW public.gislinkupdate AS
 SELECT b.identifier,
    array_to_string(array_agg(array_to_string(b.nla, ','::text)), '^'::text) AS nlink
   FROM ( SELECT x.identifier,
            x.links,
            x.rn,
            array_append(x.slx, x.lastem) AS nla,
                CASE
                    WHEN ("position"(x.lastem, 'gisdata.geothermaldata.org'::text) > 0) THEN 1
                    ELSE 0
                END AS rc
           FROM ( SELECT y.identifier,
                    y.links,
                    y.rn,
                    y.lx[1:(array_length(y.lx, 1) - 1)] AS slx,
                    regexp_replace(y.lx[array_length(y.lx, 1)], 'http://services.azgs.az.gov/ArcGIS/*'::text, 'https://gisdata.geothermaldata.org/arcgis/'::text, 'i'::text) AS lastem
                   FROM ( SELECT z.identifier,
                            z.links,
                            z.arrayorder AS rn,
                            string_to_array(z.lka, ','::text) AS lx
                           FROM ( SELECT records.identifier,
                                    records.links,
                                    t.arrayorder,
                                    t.lka
                                   FROM public.records,
                                    LATERAL unnest(string_to_array(records.links, '^'::text)) WITH ORDINALITY t(lka, arrayorder)
                                  WHERE (records.links ~~ '%services.azgs.az.gov%'::text)) z) y) x
          ORDER BY x.identifier, x.rn) b
  GROUP BY b.identifier, b.links
  WITH NO DATA;


ALTER TABLE public.gislinkupdate OWNER TO ngdsdb;
...
--
-- Name: harvest_source_hdr; Type: VIEW; Schema: public; Owner: ngdsdb
--

CREATE VIEW public.harvest_source_hdr AS
 WITH dc AS (
         SELECT md_record.dataset_id,
            count(md_record.dataset_id) AS hcount
           FROM public.md_record
          GROUP BY md_record.dataset_id
        ), cs AS (
         SELECT c.set_id,
            c.set_name,
            c.source_url,
           ...
           FROM public.collections c,
            public.schemas s
          WHERE ((c.schema_id = s.schema_id) AND (c.set_type = 'harvest'::text) AND (c.status = 'active'::text))
        ), crj AS (
         SELECT cs.set_id,
            cs.set_name,
            cs.source_url,
            ...
            cs.schema_name,
                CASE
                    WHEN (dc.hcount IS NULL) THEN (0)::bigint
                    ELSE dc.hcount
                END AS rcount
           FROM (cs
             LEFT JOIN dc ON ((dc.dataset_id = cs.set_id)))
        )
 SELECT crj.set_id,
    crj.set_name,
    crj.source_url,
...
ALTER TABLE public.harvest_source_hdr OWNER TO ngdsdb;
...
--
-- Name: qsearch; Type: FOREIGN TABLE; Schema: public; Owner: ngdsdb
--

CREATE FOREIGN TABLE public.qsearch (
    identifier text,
    title text,
    organization text,
    abstract text,
...
ALTER FOREIGN TABLE public.qsearch OWNER TO ngdsdb;
...
--
-- Name: schemacount; Type: VIEW; Schema: public; Owner: ngdsdb
--

CREATE VIEW public.schemacount AS
 WITH sc AS (
         SELECT v.source_schema_id AS sid,
            count(v.source_schema_id) AS scount
           FROM public.md_version v,
            public.md_record r
          WHERE ((v.md_id = r.md_id) AND (v.end_date IS NULL))
          GROUP BY v.source_schema_id
        )
 SELECT s.schema_id,
    s.schema_name,
    s.auth_source,
   ...
   FROM (public.schemas s
     LEFT JOIN sc ON ((s.schema_id = sc.sid)))
  WHERE (s.status = 'active'::text);


ALTER TABLE public.schemacount OWNER TO ngdsdb;
...
--
-- Name: schemapview; Type: VIEW; Schema: public; Owner: ngdsdb
--

CREATE VIEW public.schemapview AS
 WITH sm AS (
         SELECT schema_map.map_id,
            schema_map.schema_id,
            0 AS moid,
         ...
         SELECT s.map_id,
            ...
            o.omtype,
            o.ruleset_id
           FROM public.schema_object_map o,
            sp s
          WHERE (o.map_id = s.map_id)
        ), sx AS (
         SELECT sm.map_id,
            sm.schema_id,
            sm.moid,
 ...
 SELECT sx.map_id,
    ...
    sx.ruleset_id
   FROM sx
  ORDER BY sx.schema_id, sx.map_id, sx.moid;


ALTER TABLE public.schemapview OWNER TO ngdsdb;
...
--
-- Name: md_path_version_id_idx; Type: INDEX; Schema: public; Owner: ngdsdb
--

CREATE INDEX md_path_version_id_idx ON public.md_path USING btree (version_id);
...
--
-- Name: mdvnode_version_id_idx; Type: INDEX; Schema: public; Owner: ngdsdb
--

CREATE INDEX mdvnode_version_id_idx ON public.mdvnode USING btree (version_id);
